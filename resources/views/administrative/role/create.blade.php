...
<div class="row">
    <div class="col-md-6 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h6 class="card-title">Role Create</h6>
                <form class="forms-sample" action="{{route('administrative.role.store')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group">
                        <label for="exampleFormControlSelect1">Select Permissin</label>
                        <input type="checkbox" id="checkbox" >Select All
                        <select class="form-control" name="permission_id[]" id="permission_id" multiple>

                            @foreach($permission as $permissionItem)

                                <option value="{{$permissionItem['id']}}">{{ucfirst($permissionItem['name'])}}</option>
