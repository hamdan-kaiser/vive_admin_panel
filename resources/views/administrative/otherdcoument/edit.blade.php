    ...
    <div class="col-md-8 grid-margin stretch-card offset-md-2">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" action="{{route('administrative.otherdcoument.update',$data->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('type') ? 'has-danger' : '' }}">
							<label for="exampleFormControlSelect1">Select Type</label>
								<select class="form-control js-example-basic-single w-100" id="type" name="type" required style="width: 100%">
        							...
        							<option value='sop' {{ 'sop'== old('type', isset($data) ? $data->type : '')  ? 'selected' : ''}}>Sop</option>
        							<option value='cv' {{ 'cv'== old('type', isset($data) ? $data->type : '')  ? 'selected' : ''}}>Cv</option>
        							<option value='personal_reference' {{ 'personal_reference'== old('type', isset($data) ? $data->type : '')  ? 'selected' : ''}}>Personal Reference</option>
        							<option value='academic_reference' {{ 'academic_reference'== old('type', isset($data) ? $data->type : '')  ? 'selected' : ''}}>Academic Reference</option>
        						</select>
								@if($errors->has('type'))
