    ...
    <div class="col-md-8 grid-margin stretch-card offset-md-2">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" action="{{route('administrative.university.update',$data->id)}}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="location_id" id="location_id">
                            ...
                            @foreach($location as $locationItem)
                                ...
                                <option value="{{$locationItem['id']}}" {{$data->location_id == $locationItem['id'] ? 'selected' : ''}}>{{ucfirst($locationItem['title'])}}</option>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control js-example-basic-multiple" name="subject[]" id="subject" multiple="multiple">
                            ...
                            @foreach($subjects as $subject)
                                ...
                                <option value="{{$subject['id']}}" {{in_array($subject['id'],$data->subjects->pluck('id')->toArray()) ? 'selected':''}}>{{ucfirst($subject['title'])}}</option>
                        ...
                        @endif
							...
							<label for="exampleFormControlSelect1">Select Scholarship</label>
								<select class="form-control js-example-basic-single w-100" id="scholarship" name="scholarship" required style="width: 100%">
									<option selected disabled>Select your scholarship</option>
									<option value='1' {{ '1'==  old('scholarship', isset($data) ? $data->scholarship : '')   ? 'selected' : ''}}>Yes/Active</option>
									<option value='0' {{ '0'==  old('scholarship', isset($data) ? $data->scholarship : '')   ? 'selected' : ''}}>No/Inactive</option>
								</select>
