    ...
    <div class="col-md-8 grid-margin stretch-card offset-md-2">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample" action="{{route('administrative.application.update',$data->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('user_id') ? 'has-danger' : '' }}">
							<label for="exampleFormControlSelect1">Select User Id</label>
								<select class="form-control js-example-basic-single w-100" id="user_id" name="user_id"  style="width: 100%">
									<option selected disabled>Select your user_id</option>
									<option value='1' {{ '1'==  old('user_id', isset($data) ? $data->user_id : '')   ? 'selected' : ''}}>Yes/Active</option>
									<option value='0' {{ '0'==  old('user_id', isset($data) ? $data->user_id : '')   ? 'selected' : ''}}>No/Inactive</option>
								</select>
							...
							<label for="exampleFormControlSelect1">Select Course Type</label>
								<select class="form-control js-example-basic-single w-100" id="course_type" name="course_type"  style="width: 100%">
        							...
        							<option value='under_graduation' {{ 'under_graduation'== old('course_type', isset($data) ? $data->course_type : '')  ? 'selected' : ''}}>Under Graduation</option>
        							<option value='post_graduation' {{ 'post_graduation'== old('course_type', isset($data) ? $data->course_type : '')  ? 'selected' : ''}}>Post Graduation</option>
        						</select>
							...
							<label for="exampleFormControlSelect1">Select Subject Id</label>
								<select class="form-control js-example-basic-single w-100" id="subject_id" name="subject_id"  style="width: 100%">
									<option selected disabled>Select your subject_id</option>
									<option value='1' {{ '1'==  old('subject_id', isset($data) ? $data->subject_id : '')   ? 'selected' : ''}}>Yes/Active</option>
									<option value='0' {{ '0'==  old('subject_id', isset($data) ? $data->subject_id : '')   ? 'selected' : ''}}>No/Inactive</option>
								</select>
							...
							<label for="exampleFormControlSelect1">Select University Id</label>
								<select class="form-control js-example-basic-single w-100" id="university_id" name="university_id"  style="width: 100%">
									<option selected disabled>Select your university_id</option>
									<option value='1' {{ '1'==  old('university_id', isset($data) ? $data->university_id : '')   ? 'selected' : ''}}>Yes/Active</option>
									<option value='0' {{ '0'==  old('university_id', isset($data) ? $data->university_id : '')   ? 'selected' : ''}}>No/Inactive</option>
								</select>
