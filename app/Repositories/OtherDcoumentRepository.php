...
use App\Models\OtherDcoument;

class OtherDcoumentRepository
    // space that we can use the repository implements RepositoryInterface
{
    // space that we can use the repository from
    protected $model;

    public function __construct(OtherDcoument $OtherDcoument)
    {
        // set the model
        $this->model = $OtherDcoument;
    }
    public function all(){
    ...
    public function allAssociateRelationFilter($relation = [],$relationFilter = [],$filter = [],$condition = "hard",$result = 'multiple'){
         ...
         if(!empty($relationFilter)){
             ...
             foreach ($relationFilter as $relation => $items){
                 $query->whereHas($relation, function($q) use ($items)
                 {
